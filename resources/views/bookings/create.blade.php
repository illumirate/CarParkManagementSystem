                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-4">
                                    <label for="end_time" class="form-label">End Time <span
                                            class="text-danger">*</span></label>
                                    <input type="time" class="form-control" id="end_time" name="end_time" min="07:00"
                                        max="22:00" required>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch(`/api/users/${userId}/vehicles?requestId=${requestId}&timestamp=${encodeURIComponent(timestamp)}`)
                .then(response => {
                    console.log('Vehicles API raw response', response);
                ...
                .then(data => {
                    ...
                    if (data.status === 'S' && Array.isArray(data.data) && data.data.length > 0) {
                        ...
                        vehiclesApiStatus.innerHTML =
                            '<i class="fas fa-exclamation-circle text-warning"></i> No vehicles available';
                    }
                })
            ...
            `;
                ...
                document.querySelectorAll('.slot-card').forEach(card => {
                    card.addEventListener('click', function() {
                        document.querySelectorAll('.slot-card').forEach(c => c.classList.remove(
                            'border-primary', 'bg-light'));
                        this.classList.add('border-primary', 'bg-light');
                        selectSlot(this.dataset.slotId, this.dataset.slotLabel, this.dataset.zone,
                            feeFormatted);
                    });
                });
            }
            ...
            function selectSlot(slotId, slotLabel, zoneName, feeFormatted) {
                ...
                document.getElementById('summary_datetime').textContent =
                    `${dateStr}, ${formatTime12h(startTime)} - ${formatTime12h(endTime)}`;
