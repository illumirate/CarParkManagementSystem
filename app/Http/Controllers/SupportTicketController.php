...
class SupportTicketController extends Controller
{
    ...
    public function store(Request $request, SupportTicketAssignmentManager $assignmentManager): RedirectResponse
    {
        ...
        return redirect()->route('support.tickets.show', $ticket)
            ->with('success', 'Support ticket created. Our team will respond soon.');
    }
    ...
    public function show(SupportTicket $ticket): View|RedirectResponse
    {
        ...
        $this->markStaffMessagesRead($ticket);

        $ticket->load([
            'assignedTo',
            'messages' => function ($query) {
                $query->withTrashed()->where('is_internal', false)->with(['sender', 'attachments'])->oldest();
            },
        ]);
            ...
            return [
                ...
                'attachments' => $isDeleted ? [] : $m->attachments->map(function ($a) use ($ticket) {
                    return [
                        ...
                        'mime' => $a->mime_type,
                        'size' => $a->size,
                        'url' => route('support.tickets.attachments.show', [$ticket, $a]),
                    ];
                })->values(),
                'can_delete' => $canDelete,
                'delete_url' => $canDelete ? route('support.tickets.messages.delete', [$ticket, $m]) : null,
            ];
        });
        ...
        if ($request->expectsJson()) {
            ...
            return response()->json([
                'message' => [
                    ...
                    'can_delete' => $canDelete,
                    'delete_url' => $canDelete ? route('support.tickets.messages.delete', [$ticket, $message]) : null,
                ],
            ]);
        }
    ...
    private function markStaffMessagesRead(SupportTicket $ticket): void
    {
        $ticket->messages()
            ->whereNull('deleted_at')
            ->whereNull('read_at')
